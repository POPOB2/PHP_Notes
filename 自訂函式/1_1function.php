...
// function執行"計算"的結果 建議使用return接收, 而不是用echo 真要echo的話 通常function的執行內容是用於讓網頁看的 如table等 就可以用echo
        function sum($num1,$num2){
            ...
            echo "<br>";
            global $name3; //不建議使用global將function外的變數$name3引用進來,當程式多的時候 一但把function引用到其他頁面,容易有檔案遺漏問題
            return $name3.$num1+$num2;//$num1+$num2;計算完的結果使用return將結果指定給function的名稱sum 而不是變數的$sum
        }
// 若function產生的結果是要再利用 會用retur n , 若執行結果有特定目的 不做return也可以 可以使其執行完就結束 尤其是用於"顯示"



// 可以用function把引導頁面縮寫成to------------------------------------------------
// function to($url){
        function to($url='2index.php'){//預設function的to預設為index.php, 當使用to時未輸入目的地, 會導引到預設的2index.php
            header('location:'.$url);
        }
        // to('test.php');
